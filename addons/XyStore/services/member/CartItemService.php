...
use addons\XyStore\common\models\member\CartItem;
use addons\XyStore\common\interfaces\CartItemInterface;
use addons\XyStore\common\enums\PointExchangeTypeEnum;

/**
 * 已登录的购物车
 *
 * Class CartItemService
 * @package addons\XyStore\common\services
...
class CartItemService extends Service implements CartItemInterface
{
    ...
    public function create($sku, $num, $member_id)
    {
        ...
        if ($sku['product']['max_buy'] > 0) {
            // 当前购物车所有的数量
            $sum = Yii::$app->xyStoreService->memberCartItem->getSumByProductId($sku['product']['id'], $member_id) + $num;
    ...
    public function updateNum($sku, $num, $member_id)
    {
        ...
        if ($sku['product']['max_buy'] > 0) {
            // 当前购物车所有的数量
            $sum = Yii::$app->xyStoreService->memberCartItem->getSumByProductId($sku['product']['id'], $member_id);
