...
namespace addons\XyStore\services\express;
...
use addons\XyStore\common\models\express\Fee;
...
class FeeService extends Service
{
    ...
    {
        // 获取所有物流公司
        if (empty($companys = Yii::$app->xyStoreService->expressCompany->getList())) {
            return '';
        }

        // 获取默认物流公司
        if (empty($defaultCompany = Yii::$app->xyStoreService->expressCompany->getDefault())) {
