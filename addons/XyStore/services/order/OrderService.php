...
use addons\XyStore\common\enums\ExplainStatusEnum;
use addons\XyStore\common\enums\RefundStatusEnum;
use addons\XyStore\common\enums\OrderTypeEnum;
use addons\XyStore\common\models\forms\OrderQueryForm;
...
class OrderService extends \common\components\Service
{
    ...
    public function create(PreviewForm $previewForm)
    {
        ...
        if ($order->shipping_type == ShippingTypeEnum::VISIT) {
            Yii::$app->xyStoreService->orderPickup->create($previewForm->pickup, $order);
        }

        // 发票记录
        if (!empty($previewForm->invoice)) {
            Yii::$app->xyStoreService->orderInvoice->create($order, $previewForm->invoice, $previewForm->invoice_content);
            $order->invoice_id = $previewForm->invoice->id;
        }

        // 使用优惠券
        !empty($previewForm->coupon) && Yii::$app->xyStoreService->marketingCoupon->used($previewForm->coupon, $order->id);
        // 创建订单详情
    ...
    public function pay(Order $order, $paymentType)
    {
        ...
        Yii::$app->xyStoreService->productSku->decrRepertory($skuNums);
    }
...
//        }
    ...
    public function closeWholesaleAll()
    {
        $orders = Order::find()
            ...
            ->where(['in', 'wholesale_id', Yii::$app->xyStoreService->marketingWholesale->findLoseEfficacy()])
            ->with('product')
            ->asArray()
            ->all();
        ...
        try {
            foreach ($orders as $order) {
                ...
                if ($order['order_status'] == OrderStatusEnum::NOT_PAY) {
                    ...
                    // 退款进订单
                    $orderProduct = Yii::$app->xyStoreService->orderProduct->refundReturnMoney($product['id']);
                    // 退款进用户余额/原路退回
    ...
    {
        $orderProducts = Yii::$app->xyStoreService->orderProduct->findByOrderId($order_id);
    ...
    {
        $orderProduct = Yii::$app->xyStoreService->orderProduct->findByOrderId($order_id);
...
//        }
    ...
    public function getOrderCountGroupByMemberId($member_id)
    {
        ...
        $memberStatus[3] = Order::find()
            ...
            ->scalar();
        // 售后
        $memberStatus[-1] = Yii::$app->xyStoreService->orderProduct->getAfterSaleCount($member_id);
