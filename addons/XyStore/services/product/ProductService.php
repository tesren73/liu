...
use addons\XyStore\common\models\forms\ProductSearch;
use addons\XyStore\common\models\base\Spec;
...
class ProductService extends Service
{
    ...
    {
        $cates = Yii::$app->xyStoreService->memberFootprint->findCateIdsByMemberId($member_id);
    ...
    public function getListBySearch(ProductSearch $search)
    {
        ...
        if ($cate_id = $search->cate_id) {
            $cate_ids = Yii::$app->xyStoreService->productCate->findChildIdsById($cate_id);
        }
    ...
    public function getSpecValueAttribute(Product $model)
    {
        ...
        // 获取基础属性
        $baseAttribute = Yii::$app->xyStoreService->baseAttribute->getDataById($model->base_attribute_id);
    ...
    protected function getSpecValue($model, $spec_ids)
    {
        ...
        $baseSpecValue = Yii::$app->xyStoreService->baseSpec->getListWithValueByIds($spec_ids);
