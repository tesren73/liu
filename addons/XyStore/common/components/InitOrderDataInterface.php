...
use addons\XyStore\common\enums\ProductMarketingEnum;
use addons\XyStore\common\enums\PointExchangeTypeEnum;
use addons\XyStore\common\traits\CalculatePriceTrait;
        ...
        foreach ($groupOrderProducts as $product_id => &$groupOrderProduct) {
            ...
            if (!in_array($type, [PointExchangePurchase::getType()])) {
                ...
                if ($previewForm->member->current_level > 0) {
                    $memberDiscount = Yii::$app->xyStoreService->productMemberDiscount->findByProductIdAndLevel($product_id, $previewForm->member->current_level);
